...
int depth(TreeNode<int>* root,TreeNode<int>* find ){
	...
	{
		deep= depth (root->children[i], find);
	}
	return deep +1;
...
void replaceWithDepthValue(TreeNode<int>* root) {
    ...
    queue <TreeNode<int>*> replace ;
    replace.push(root);
    root->data = 0;
    while(replace.front()!=NULL){
    	TreeNode<int>* node = replace.front();
    	replace.pop();
    	int change = depth(root,node)+1;
    	cout<<change<<" "<<node->data<<endl;
    	for (int i = 0; i < node->children.size(); i++){
    		node->children[i]->data=change;
    		replace.push(node->children[i]);
		}
